...
class ClassBoard extends GetView<ClassController> {
  ClassBoard({Key key}) : super(key: key);
                    ...
                    children: [
                      Padding(
                        ...
                        child: ElevatedButton(
                            onPressed: () {
                              classController.boardIndex.value = 0;
                ...
                Expanded(
                  child: Obx(() {
                    List<bool> dataAvailable = [
                      classController.dataAvailableRecruitPreview
                    ];

                    if (dataAvailable[classController.boardIndex.value]) {
                      // return Obx(() => ListView(
                      ...
                      return ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            return RecruitPostPreview(
                                body: classController.postBody[index]);
                          });
                    } else {
                      // navigate로 왔는지
                      if (Get.parameters.isEmpty) {
                        classController.type('1');
                        classController.page('1');
                      } else {
                        classController.type(Get.parameters['COMMUNITY_ID']);
                        classController.page(Get.parameters['page']);
                      }
